...
## e.g.
## ./zoofxb3p.sh - b C5 -f /path/to/filename.b3p -p 1

## known issues:
## accept only values between 0 - 127. Comes into conflict with Total PATCH LEVEL/Tempo/and maybe some patches.
## sometimes it handles the patchnames incorrectly. A Problem with space characters, it shows the directory, 
## so it can't be transformed into a space. 
...
prod=$(xmlstarlet sel -t -v "(/PatchData/Product)" -n "$4")
echo $prod
...
## Patchname
pname=$(xmlstarlet sel -t -v "(/PatchData/Name)" -n "$4")
echo $pname
pname0=("${pname:0:1}")
echo $pname0
...
m3p1=$(xmlstarlet sel -t -v "(/PatchData/Module3/Prm1)" -n "$4")
